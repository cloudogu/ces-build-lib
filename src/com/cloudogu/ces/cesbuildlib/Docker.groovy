...
class Docker implements Serializable {
    ...
    //See UnitTest for more infos about the capabilities of this method
    static parseRegistryImageNameAndTagFromString(imageString) {
        def matcher = (imageString =~ /(.+?\..+?\/)?(.+?\/)?([^:]+):(.+)/)
        matcher.find() ?: { throw new IllegalArgumentException("Invalid image format: ${imageString}") }()

        def registry = matcher.group(1)?.replaceAll("/\$", "") ?: "docker.io"
        def imageName = (matcher.group(2) ?: "") + matcher.group(3)
        def tag = matcher.group(4)

        [registry: registry, imageName: imageName, tag: tag]
